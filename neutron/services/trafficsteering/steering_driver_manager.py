...
class SteeringDriverManager(stevedore.named.NamedExtensionManager):
        ...
        for driver in self.ordered_steering_drivers:
            try:
                ...
                if not continue_on_failure:
                    break
        if error:
            raise ts_exc.SteeringDriverError(
                method=method_name
            )
    ...
    def create_steering_classifier_precommit(self, context):
        """Notify all steering drivers during classifier creation.

        :raises: neutron.plugins.trafficsteering.common.SteeringDriverError
        if any steering driver create_steering_classifier_precommit call fails.
        ...
        that all steering drivers are called in this case.
        """
        self._call_on_drivers("create_steering_classifier_precommit", context)

    def create_steering_classifier_postcommit(self, context):
        """Notify all steering drivers after classifier creation.

        :raises: neutron.plugins.trafficsteering.common.SteeringDriverError
        if any steering driver create_steering_classifier_postcommit call fails.

        Called after the database transaction. If a steering driver
        raises an exception, then a SteeringDriverError is propagated
        to the caller, where the classifier will be deleted, triggering
        any required cleanup. There is no guarantee that all steering
        drivers are called in this case.
        """
        self._call_on_drivers("create_steering_classifier_postcommit", context)

    def update_steering_classifier_precommit(self, context):
        """Notify all steering drivers during classifier update.

        :raises: neutron.plugins.trafficsteering.common.SteeringDriverError
        if any steering driver update_steering_classifier_precommit call fails.
        ...
        that all steering drivers are called in this case.
        """
        self._call_on_drivers("update_steering_classifier_precommit", context)

    def update_steering_classifier_postcommit(self, context):
        """Notify all steering drivers after classifier update.

        :raises: neutron.plugins.trafficsteering.common.SteeringDriverError
        if any steering driver update_steering_classifier_postcommit call fails.
        ...
        then reraised at the end to notify the caller of a failure.
        """
        self._call_on_drivers("update_steering_classifier_postcommit", context,
                              continue_on_failure=True)

    def delete_steering_classifier_precommit(self, context):
        """Notify all steering drivers during classifier deletion.

        :raises: neutron.plugins.trafficsteering.common.SteeringDriverError
        if any steering driver delete_steering_classifier_precommit call fails.
        ...
        that all steering drivers are called in this case.
        """
        self._call_on_drivers("delete_steering_classifier_precommit", context)

    def delete_steering_classifier_postcommit(self, context):
        """Notify all steering drivers after classifier deletion.

        :raises: neutron.plugins.trafficsteering.common.SteeringDriverError
        if any steering driver delete_steering_classifier_postcommit call fails.
        ...
        then reraised at the end to notify the caller of a failure. In
        general we expect the caller to ignore the error, as the
        classifier resource has already been deleted from the database
        and it doesn't make sense to undo the action by recreating the
        classifier.
        """
        self._call_on_drivers("delete_steering_classifier_postcommit", context,
                              continue_on_failure=True)
